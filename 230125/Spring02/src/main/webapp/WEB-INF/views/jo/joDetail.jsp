...
<table width=100%> 
	<tr bgcolor="LavenderBlush" height="30">
		<th>I D</th><th>Password</th><th>Name</th><th>Info</th>
		<th>Birthday</th><th>Jno</th><th>Age</th><th>Point</th>
	...
	<c:if test="${not empty banana}">
		...
		<tr height="30">
			...
			<td>${member.jno}</td><td>${member.age}</td>
			<td>${member.point}</td><td>${member.rid}</td>
			<td><img alt="memberImage" src="${member.uploadfile}" width=50 height=60></td>
		</tr>	
		</c:forEach>
	</c:if>
	<c:if test="${empty banana}">
	<tr><td colspan="1o">** 출력할 자료가 1건도 없습니다. **</td></tr>
	</c:if>
</table>
